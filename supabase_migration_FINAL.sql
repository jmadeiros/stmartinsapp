-- ============================================================================
-- The Village Hub - Complete Database Migration (FINAL)
-- ============================================================================
-- Architecture:
...
CREATE TYPE app.post_category AS ENUM ('intros', 'wins', 'opportunities', 'questions', 'learnings', 'general');

-- Simplified reactions (just like for initial build)
CREATE TYPE app.reaction_type AS ENUM ('like');
...
CREATE TABLE app.posts (
  ...
  category app.post_category NOT NULL DEFAULT 'general',
  image_url TEXT,
...
CREATE TABLE app.post_reactions (
  ...
  post_id UUID NOT NULL REFERENCES app.posts(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  reaction_type app.reaction_type NOT NULL DEFAULT 'like',
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
ALTER TABLE app.posts
  ...
  FOREIGN KEY (linked_event_id) REFERENCES app.events(id) ON DELETE SET NULL;

ALTER TABLE app.posts
  ADD CONSTRAINT fk_posts_linked_project
  FOREIGN KEY (linked_project_id) REFERENCES app.projects(id) ON DELETE SET NULL;

-- Add FK for events linking to parent project
ALTER TABLE app.events
  ADD CONSTRAINT fk_events_parent_project
  FOREIGN KEY (parent_project_id) REFERENCES app.projects(id) ON DELETE SET NULL;

-- Add FK for meeting notes linking to events
ALTER TABLE app.meeting_notes
...
CREATE INDEX idx_posts_opportunities ON app.posts(category, created_at DESC) WHERE category = 'opportunities' AND deleted_at IS NULL;
...
CREATE INDEX idx_events_parent_project ON app.events(parent_project_id) WHERE parent_project_id IS NOT NULL;
...
-- Project Interest
CREATE INDEX idx_project_interest_project ON app.project_interest(project_id);
CREATE INDEX idx_project_interest_org ON app.project_interest(org_id);
...
CREATE INDEX idx_conversations_created_by ON app.conversations(created_by);
...
CREATE OR REPLACE FUNCTION app.count_project_events(p_project_id UUID)
RETURNS INTEGER AS $$
BEGIN
  RETURN (
...
DECLARE
  ...
  v_user_id UUID;
BEGIN
  INSERT INTO app.conversations (is_group, name, org_id, created_by)
...
CREATE OR REPLACE FUNCTION app.rsvp_event(
  p_event_id UUID,
  p_org_id UUID,
  p_status TEXT,
...
RETURNS VOID AS $$
BEGIN
  INSERT INTO app.project_interest (
...
CREATE OR REPLACE FUNCTION app.generate_event_ical(p_event_id UUID)
RETURNS TEXT AS $$
DECLARE
  v_event RECORD;
  v_ical TEXT;
