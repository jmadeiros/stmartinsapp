...
/**
 * OPTION 2: Split into Functional Blocks
 * Break the bar into 2-3 smaller cards that serve different purposes
 */
...
export function MonthlyCalendarOption2({ events, initialDate }: MonthlyCalendarProps) {
  ...
  const nextEvent = useMemo(() => {
    const now = new Date()
    return sortedEvents.find(event => new Date(event.start) > now) ?? null
  }, [sortedEvents])
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_380px]">
      ...
      <div className="space-y-4">
        {/* OPTION 2: Three Functional Cards */}
        <motion.div
        ...
        >
          {/* Card 1: Month & Navigation */}
          <div className="rounded-2xl border border-gray-100 bg-white shadow-md p-4 flex flex-col justify-between">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">{format(currentMonth, "yyyy")}</p>
            </div>
          ...
          {/* Card 2: Quick Stats & Next Event */}
          <div 
            className="rounded-2xl p-4 relative overflow-hidden"
            style={{
              backgroundImage:
                "linear-gradient(135deg, oklch(0.6 0.118 184.704), oklch(0.52 0.12 166 / 0.9))",
            }}
          >
            ...
            <div className="relative z-10">
              ...
              <p className="text-3xl font-bold text-white">{eventsInCurrentMonth.length}</p>
              <p className="text-sm text-white/80">events scheduled</p>
              {nextEvent && (
                <div className="mt-3 pt-3 border-t border-white/20">
                  <p className="text-xs text-white/70 uppercase tracking-wide mb-1">Next up</p>
                  <p className="text-sm font-medium text-white truncate">{nextEvent.title}</p>
                  <p className="text-xs text-white/70">{format(new Date(nextEvent.start), "MMM d, h:mma")}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Card 3: Quick Action */}
          <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50/50 p-4 flex flex-col items-center justify-center text-center hover:border-primary/50 hover:bg-primary/5 transition-colors cursor-pointer group">
            <div className="p-3 rounded-xl bg-white shadow-sm group-hover:shadow-md transition-shadow mb-3">
