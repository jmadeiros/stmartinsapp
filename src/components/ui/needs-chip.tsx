import { HandHeart, Users, Package, DollarSign, type LucideIcon } from "lucide-react"

import { cn } from "@/lib/utils"
import type { Needs, ParticipantRequest } from "@/lib/social/types"
...
export function NeedsChips({ needs, status, className }: NeedsChipsProps) {
  ...
  if (needs.participantRequests && needs.participantRequests.length > 0) {
    needs.participantRequests.forEach((req: ParticipantRequest) => {
      const label = req.count ? `${req.programTag} (${req.count})` : req.programTag
