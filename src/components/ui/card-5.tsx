...
export const HighlightCard = React.forwardRef<HTMLDivElement, HighlightCardProps>(
  ({ title, description, metricValue, metricLabel, buttonText, onButtonClick, icon, color = "default", className, children }, ref) => {
    const theme = colorThemes[color] || colorThemes.default

    return (
      <motion.div
        ref={ref}
        className={cn("relative w-full max-w-md overflow-hidden rounded-2xl p-5 pb-4 shadow-lg text-white", className)}
        style={
          {
      ...
      >
        ...
        <div className="flex h-full flex-col justify-between min-h-0 gap-2.5">
          <div className="flex-shrink min-h-0">
            <motion.h3 variants={itemVariants} className="text-2xl font-bold tracking-tight pr-14">
              {title}
            </motion.h3>
            {description && (
          ...
          <div
            className={cn(
              "flex items-end justify-between gap-2.5 flex-shrink-0",
              !(metricValue || metricLabel) && "justify-end"
            )}
          >
            {(metricValue || metricLabel) && (
              <motion.div variants={itemVariants}>
                {metricValue && <p className="text-3xl font-bold tracking-tighter">{metricValue}</p>}
                {metricLabel && <p className="text-xs opacity-90">{metricLabel}</p>}
              </motion.div>
            )}
            <motion.button
              variants={itemVariants}
              onClick={onButtonClick}
              className="rounded-full bg-white/30 px-3 py-1.5 text-xs font-semibold backdrop-blur-sm transition-colors hover:bg-white/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 shrink-0"
