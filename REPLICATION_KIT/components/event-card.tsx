...
export function EventCard({ event }: EventCardProps) {
  ...
  const handleAttendToggle = () => {
    ...
    // Calculate popover position after state update
  ...
  const calculatePopoverPosition = () => {
    ...
    // Check horizontal space: prefer right, flip to left if needed
    const spaceOnRight = viewportWidth - buttonRect.right
    const spaceOnLeft = buttonRect.left
    const alignRight = spaceOnRight >= popoverWidth || spaceOnRight > spaceOnLeft

    // Check vertical space: strongly prefer down, only flip up if really necessary
    const spaceBelow = viewportHeight - buttonRect.bottom
    const spaceAbove = buttonRect.top
    // Only flip to top if there's less than 200px below AND significantly more space above
  ...
  return (
    <Card className="border border-border bg-card shadow-sm transition-shadow hover:shadow-md">
      <div className="p-6">
        ...
        {/* Event Details Box */}
        <div className="mb-4 space-y-2 rounded-xl bg-muted/50 p-4 border border-border/50">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {event.needs.participantRequests && event.needs.participantRequests.length > 0 && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 px-3 py-1 text-xs font-medium text-blue-700 border border-blue-200">
                  ...
                  Participants: {event.needs.participantRequests.map(req => 
              ...
              <AnimatePresence initial={false}>
                {attending && supportPanelOpen && (
                  <motion.div
                    ...
                    className={cn(
                      "absolute z-30 w-64 rounded-lg border border-blue-200/60 bg-white/98 p-3 shadow-lg backdrop-blur-sm",
                      ...
                      {hasNeeds ? (
                        <div className="space-y-1.5">
                          ...
                          {event.needs?.participantRequests &&
                            event.needs.participantRequests.length > 0 && (
                              <label className="flex items-start gap-2.5 rounded-md border border-transparent bg-blue-50/50 p-2.5 text-left transition hover:border-blue-200 hover:bg-blue-50 cursor-pointer">
                                <input
                                  type="checkbox"
